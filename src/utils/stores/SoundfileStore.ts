...
type SoundfileState = {
  soundfile: Soundfile[];
  fetchSoundfiles: () => void;
  createSoundfileFromBlob: (audioBlobUrl: string, key: string) => Promise<void>;
  createSoundfileFromFile: (file: any) => void;
  updateSoundfile: (id: number, updatedData: any) => void;
  deleteSoundfile: (id: number) => void;
  getSoundfileById: (id: number) => Soundfile | undefined;
  setSoundfiles: (data: Soundfile[]) => void;
};

export const useSoundfileStore = create<SoundfileState>((set, get) => ({
  soundfile: [],

  fetchSoundfiles: () => {
    if (auth.getAuthUser() !== null) {
      http.get('/api/sound-files')
        .then(response => get().setSoundfiles(response.data))
        .catch(error => console.error('Ошибка при получении данных:', error));
    }
  },

  createSoundfileFromBlob: async (audioBlobUrl, key) => {
    ...
    try {
      ...
      await http.post('/api/sound-files', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      ...
      get().fetchSoundfiles();
      console.log('Upload successful');
      show('Файл добавлен', 'success');
    } catch (error) {
      console.error('Error uploading file', error);
      show('Ошибка при добавлении файла', 'danger');
    }
  },

  createSoundfileFromFile: (file) => {
    const show = useToastStore.getState().show;
    const formData = new FormData();
    formData.append("file", file);
  ...
  updateSoundfile: (id, updatedData) => {
    const show = useToastStore.getState().show;
    http.put(`/api/sound-files/${id}`, updatedData, {
      headers: {
        'Content-Type': 'application/json',
  ...
  deleteSoundfile: (id) => {
    ...
    http.delete_(`/api/sound-files/${id}`)
      .then(() => {
        get().fetchSoundfiles();
        show('Данные удалены', 'success');
      })
      .catch(error => {
        console.error('Ошибка при удалении:', error);
        show('Ошибка при удалении данных', 'danger');
      });
  },

  getSoundfileById: (id) => {
    return get().soundfile.find(order => order.id === id);
  },
